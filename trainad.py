...
from datasets.train import build_training_dataset
...
import models.reconstruction.models
import warnings
...
def get_args():
    parser = argparse.ArgumentParser('Mvtec', add_help=False)
    ...
    parser.add_argument('--mask_ratio', default=0.3, type=float,
                        help='number of the visual patches need be masked')
...
def main(args):
    ...
    args.window_size =model.window_size
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            args, effmodel, model, train_loader,
...
def train_one_epoch(args, effmodel, model: torch.nn.Module,
    ...
    for step, (batch, target) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            input = {"image": token_images}
            tokenimg_fe = effmodel(input)

            input = {"image": images}
            mskimg_fe = effmodel(input)

            outputs = model(mskimg_fe, tokenimg_fe, bool_masked_pos)

            loss = outputs["mseLoss"]
            uncertainty = loss
        ...
        mlm_acc = (uncertainty.detach().cpu()).float().mean().item()
